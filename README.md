...
![CI](https://github.com/Es777777/aegis-code-repo-agent/actions/workflows/ci.yml/badge.svg)
![Python](https://img.shields.io/badge/Python-3.11%2B-blue)
![License](https://img.shields.io/badge/License-MIT-green.svg)
![Competition](https://img.shields.io/badge/Volcano%20Cup-Agent%20Ready-orange)
...
- [Competition Demo](docs/DEMO.md)
- [Demo Results](docs/DEMO_RESULTS.md)
- [Release Checklist](docs/RELEASE_CHECKLIST.md)
- [Architecture](docs/ARCHITECTURE.md)
- [Skill Entry](skills/aegis-repo-analyst/SKILL.md)

## Why AEGIS

- 面向比赛和演示：离线优先，内置 `doctor`、`eval`、`ready`、`handoff` 主路径。
- 面向多智能体接力：通过 `handoff_card.json`、`run_summary.json` 和 `--from-output` 复用分析产物。
- 面向真实问答：RAG 不只给摘要，而是把带行号的真实源码块塞进上下文。
- 面向工程验证：内置评测、工件契约、manifest 完整性校验和 readiness gate。
...
- `report.html`: 给人看的仓库分析报告
- `run_summary.json`: 给编排器和诊断流程的完整状态账本
- `handoff_card.json`: 给下游 Agent 直接接力的默认机器接口
...
- [docs/DEMO.md](docs/DEMO.md): 最短可复现演示路径
- [docs/DEMO_RESULTS.md](docs/DEMO_RESULTS.md): 当前仓库状态下的真实演示结果摘要
- [docs/RELEASE_CHECKLIST.md](docs/RELEASE_CHECKLIST.md): 发布/提交前检查清单
...
LLM 只接收 Context Router 选出的最小必要上下文，输出仍会进入 Evidence Reviewer。Context Router 会优先放入带行号的真实源码上下文，并标记 `Complete file: yes/no`，避免 LLM 只看到文件摘要或符号列表。
...
- `prompt_context_coverage`：RAG 问答是否能生成可直接放入 LLM prompt 的源码上下文包。
- `complete_file_context_coverage`：上下文包是否至少包含一个期望文件的完整带行号源码。
- `prompt_context_expected_path_coverage`：期望文件路径进入 prompt 上下文的比例。
- `complete_file_expected_path_coverage`：期望文件路径以完整文件形式进入上下文的比例。
- `overall_score`：综合分。
...
Artifact inventory entries now also declare a small reuse contract:
`produced_by`, `depends_on`, `optional_depends_on`, `reusable`, and a short
description. Downstream agents can use this to decide whether an artifact is a
primary input, a derived handoff file, or a post-run byproduct that should only
be trusted when its prerequisites are present.
...
It now also includes `artifact_contracts`, which reports dependency contracts
for each tracked artifact, whether those contracts are currently satisfied, and
whether the directory is reusable without regenerating missing prerequisites.
The `orchestration` object is the machine-readable next-step guide:

- `recommended_command`: next command category such as `ask`, `eval`, `ready`, or `none`
- `recommended_args`: ready-to-run CLI argument list for that next command
- `recommended_command_line`: shell-friendly command string
- `why_recommended`: concise explanation of what is missing or what failed
- `recovery_commands`: fallback commands when reuse is blocked or a fresh analysis is needed
- `requires_fresh_analysis`: `true` when the missing root artifacts mean another full analysis run is required
- `repair_ready` / `blocking_issues_count`: whether a structured repair plan is available and how many blocking items it contains
- `primary_repair_step`: the first machine-readable remediation step another agent should try
- `repair_plan`: ordered remediation items with category, severity, affected commands, and recommended CLI command
  Failed evaluation items also expose failing RAG/trace cases, and failed readiness items expose failing checks.
- `can_reuse_for` / `blocked_by`: command-level reuse gate summary for downstream agents

When using `--json`, the main CLI payload now also echoes this handoff block as
top-level `orchestration` plus full `summary`, so a UI or orchestrator does not
need to reopen `run_summary.json` just to decide the next action.
Use `python main.py --from-output output\aegis\<repo-name> --status --json`
when another agent only needs the current handoff state, manifest-integrity
result, artifact-contract summary, and command-level reuse gates without
creating new QA/evaluation/readiness artifacts.
...
fails instead of answering from stale evidence. When the output directory
already contains tracked post-run artifacts such as `run_summary.json`,
`qa_answer.json`, `context_pack.md`, `llm_prompt.md`, `evaluation.json`,
`impact.json`, or `readiness.json`, AEGIS verifies those hashes too before
allowing reuse.
If another agent wants to know whether the directory is semantically reusable,
not just hash-clean, it should inspect `run_summary.json.artifact_contracts`
and confirm that present artifacts have all required dependencies available.
AEGIS also applies a command-specific contract gate during `--from-output`
reuse: `--ask` requires the saved RAG inputs and rejects dangling prompt files,
`--impact` ignores unrelated missing QA byproducts, and `--ready` reuses the
analysis outputs plus RAG inputs but can regenerate `evaluation.json` during the
same run. This keeps reuse strict where semantics matter without turning old
optional byproducts into global blockers.
...
AEGIS writes two downstream orchestration artifacts:

- `handoff_card.json`: the default machine interface with the primary task,
  recommended action, reusable artifact pointers, and the best available
  investigation brief. When the active task is a QA investigation, the card
  also carries a ready-to-run `primary_task.recommended_command_line` plus the
  concrete `recommended_args` needed to replay the ask workflow with the same
  context contract. Downstream agents should execute
  `primary_task.recommended_command_line` first when it is present.
- `run_summary.json`: the deeper orchestration ledger with artifact contracts,
  repair steps, command reuse gates, and next actions.

Use the minimal handoff interface when another agent only needs the execution
card:

```powershell
python main.py --from-output output\aegis\<repo-name> --handoff --json
```

Use the full status interface when another agent also needs orchestration and
repair diagnostics:
...
When `--from-output` validates an existing directory, AEGIS verifies tracked
post-run artifacts including `run_summary.json` and `handoff_card.json` before
allowing reuse.
