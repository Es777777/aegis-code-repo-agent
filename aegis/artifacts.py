...
def load_analysis_result(output_dir: Path) -> AnalysisResult:
    if not output_dir.exists():
        raise ArtifactLoadError(f"Analysis output directory does not exist: {output_dir}")
    if not output_dir.is_dir():
        raise ArtifactLoadError(f"Analysis output path is not a directory: {output_dir}")
    _require(output_dir / "knowledge.json")
...
def load_knowledge(path: Path) -> RepoKnowledge:
    ...
    try:
        ...
        raise ArtifactLoadError(f"Invalid knowledge artifact {path}: {exc}") from exc
...
def load_findings(path: Path) -> list[Finding]:
    ...
    try:
        ...
        raise ArtifactLoadError(f"Invalid findings artifact {path}: {exc}") from exc
...
def load_rag_index(path: Path) -> RAGIndex:
    ...
    try:
        ...
        raise ArtifactLoadError(f"Invalid RAG index artifact {path}: {exc}") from exc
...
def _read_json(path: Path) -> Any:
    ...
    try:
        ...
        raise ArtifactLoadError(f"Could not read artifact {path}: {exc}") from exc
    except json.JSONDecodeError as exc:
        raise ArtifactLoadError(f"Artifact is not valid JSON: {path} ({exc})") from exc
...
def _require(path: Path) -> None:
    if not path.exists():
        raise ArtifactLoadError(
            ...
            "Run a full analysis first or pass the correct --from-output directory."
        )
    if not path.is_file():
        raise ArtifactLoadError(f"Required artifact is not a file: {path}")
